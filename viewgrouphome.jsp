...
<% 
	...
	String is_a = "-";
	 ...
	 try {
		...
		PreparedStatement cps = con.prepareStatement("select is_approved from group_sharing where user_email=? and group_name = (select group_name from grp where group_id = ?)");
		cps.setString(1, curr_email);
		cps.setInt(2, group_id);
...
/* 	FileReader reader=new FileReader("/opt/tomcat/apache-tomcat-8.5.83/webapps/config.properties");  
	...
	try {
		Class.forName("com.mysql.jdbc.Driver");
		Connection con = DriverManager.getConnection("jdbc:mysql://" +url+ "/" + db,user,pass);
/* 		String g_id = request.getSession().getAttribute("id").toString(); */
		String user_id = request.getSession().getAttribute("user_id").toString();
		PreparedStatement gs = con.prepareStatement("select * from grp where group_id = ?");
		gs.setInt(1, group_id);
		PreparedStatement ps = con.prepareStatement("select * from document where id in (select doc_id from sharing where group_name = (select group_name from grp where group_id = ?))");
		ps.setInt(1, group_id);
		PreparedStatement us = con.prepareStatement("select * from user where id=?");
		us.setString(1, user_id);
		
		PreparedStatement gus = con.prepareStatement("select * from grp where group_id=?");
		gus.setInt(1, group_id);
...
/* 		if(request.getParameter("search") != null){
		...
		ResultSet rgs = gs.executeQuery();
		ResultSet rs = ps.executeQuery();
		ResultSet urs = us.executeQuery();
		ResultSet gurs = gus.executeQuery();
		urs.next();
		gurs.next();
		rgs.next();
	%>
		...
		<div class="container">
			<div class="row">
				<div class="col-sm">
					...
					<h3><%=rgs.getString("group_name")%></h3>
				</div>
				<div class="col-sm">
				<form id="picture" action="GroupPictureServlet" method="POST">
					<% String dp = gurs.getString("display_picture");
					if(dp != null){ %>
						<img class="img-circle" id="imgFileUpload" title="Change Picture" src="<%=gurs.getString("display_picture") %>" style="cursor: pointer;width: 50%;border-radius: 20%">
						<span id="spnFilePath"></span>
						<input type="hidden" name= "pic" id = "pic"/>
						<input type="hidden" name= "group_id" id = "group_id" value="<%=gurs.getString("group_id")%>"/>
					...
					<% 		}
						...
						<img class="img-circle" id="imgFileUpload" title="Change Picture" src="images/grouphome.png" style="cursor: pointer;">
						<span id="spnFilePath"></span>
						<input type="hidden" name= "pic" id = "pic"/>
						<input type="hidden" name= "group_id" id = "group_id" value="<%=gurs.getString("group_id")%>"/>
   						<input type="file" id="FileUpload1" style="display: none" accept="image/png, image/gif, image/jpeg"  onchange="encodeImageFileAsURL(this)" />
